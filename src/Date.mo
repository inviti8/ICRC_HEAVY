...
module Date {
  ...
  public func show(date : Date) : Text {
    DateTypes.showDateParts(unpack(date))
  };
  ...
  public func create(year : Year, month : Month, day : Day) : ? Date {
    ...
    if (0 <= days and days <= 2932896) {
      let DateTypes = Int32.fromInt(days);
      ? #Date DateTypes
    } else {
      null
    }
  ...
  public func now() : Date {
    let DateTypes = Int32.fromInt(Nat64.toNat(Prim.time()) / 86400000000000);
    #Date DateTypes
  };
   ...
   */
  public func unpack(#Date DateTypes : Date) : DateParts {
    var year = 1970;
    var days = Int32.toInt(DateTypes);
    var leap = false;
    var size = 365;
    while (days >= size) {
      year += 1;
    ...
    let wday = switch (1 + (Int32.toInt(DateTypes) + 4) % 7) {
      case 1 #Sunday;
      case 2 #Monday;
      case 3 #Tuesday;
      case 4 #Wednesday;
   ...
   */
  public func isFutureDate(#Date DateTypes : Date) : Bool {
    let now = Int32.toInt(Int32.fromInt(Nat64.toNat(Prim.time()) / 86400000000000));
    let days = Int32.toInt(DateTypes);
   ...
   */
  public func isPastDate(#Date DateTypes : Date) : Bool {
    let now = Int32.toInt(Int32.fromInt(Nat64.toNat(Prim.time()) / 86400000000000));
    let days = Int32.toInt(DateTypes);
